...
def login_Model():
    ...
    return {
        'get_stored_hashed_password': get_stored_hashed_password,
        'get_user_status': get_user_status,
        'get_loa': get_loa
    }

# Controller
def login_Controller(model):
    def check_credentials(username, password, user_entry, pass_entry, window):
        user_status = model['get_user_status'](username)
        ...
        salt, stored_hashed_password = model['get_stored_hashed_password'](username)

        if salt and stored_hashed_password:
            hashed_password = hash_password(password, salt)
            if hashed_password == stored_hashed_password:
                loa = model['get_loa'](username)
                if loa is not None:
...
def login_View(controller):
        ...
        if 'Logo' in abouts:
            try:
                ...
                if isinstance(logo_base64, str):
                    print("Base64 data length:", len(logo_base64))  
                    print("Base64 data sample:", logo_base64[:100])

                    try:
                        logo_data = base64.b64decode(logo_base64)
                        print("Decoded data length:", len(logo_data))
    ...
    def create_login_window():
        ...
        canvas.tag_bind(forgot_pass, "<Button-1>", lambda event: controller['go_to_window']("forgot pass"))
        ...
        canvas.tag_bind(about, "<Button-1>", lambda event: controller['go_to_window']("about_login"))
        ...
        login_ = Button(text="Login", font=("Hanuman Regular", 14), command=lambda: controller['check_credentials'](user_entry.get(), pass_entry.get(), user_entry, pass_entry, window), bg="#FF7676", fg='white', relief="raised")
...
if __name__ == "__main__":
    model = login_Model()
    controller = login_Controller(model)
    view = login_View(controller)
    view()
