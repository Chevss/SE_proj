...
class pos_Model: #****
    ...
    def check_product_quantities(self, purchase_list):
    ...
    def check_product_availability(self, purchase_list):
        try:
            for item in purchase_list:
    ...
    def update_inventory(self, purchase_list):
        try:
            # Start a transaction
    ...
    def insert_purchase_history(self, purchase_list, total_amount, customer_money, change, cashier_name, customer_name, purchase_id):
        try:
...
class pos_Controller:
    ...
    def __init__(self):
        self.model = pos_Model(self)
        self.view = pos_View(self)

    def process_barcode(self, barcode_value):
        """Handles the logic for processing a barcode."""
        result = model.search_barcode(barcode_value)
        ...
        if result:
            ...
            view.update_purchase_display(self)

            # Update the total label
            view.update_total_label(self)
    ...
    def go_to_window(self, window_type):
        ...
        if window_type == "logout":
            user_logs.log_actions(shared_state.current_user, "Logged Out")
            shared_state.current_user = None
            shared_state.current_user_loa = None
            import login
            login.create_login_window(self)
        elif window_type == "inventory":
            import products
            products.create_products_window(self)
        elif window_type == "register":
            import registration
            registration.create_registration_window(self)
        elif window_type == "barcode":
            import barcode_ad
            barcode_ad.create_barcode_window(self)
        elif window_type == "backup_restore":  
            import backup_restore
            backup_restore.create_backup_restore_window(self)
        elif window_type == "reports":  
            import reports
            reports.create_reports_window(self)
        elif window_type == "help":
            import help_ad
            help_ad.create_help_window(self)
        elif window_type == "about":
            import about
            about.create_about_window(self)
        elif window_type == "manual":
            import user_manual
            user_manual.create_users_manual_window(self)
    
    def go_to_return(self):
        if shared_state.current_user_loa == "admin":
            import return_item
            return_item.create_return_item_window()
        else: view.prompt_admin_credentials()
    
    def process_purchase(self, cashier_name, customer_name, customer_contact, customer_money, purchase_window):
        try:
        ...
        view.create_receipt(cashier_name, customer_name, customer_contact, customer_money, change, purchase_list)
        ...
        model.insert_purchase_history(purchase_list, total_amount, customer_money, change, cashier_name, customer_name, purchase_id)

        action = "Checked out and generated PDF file for the receipt."
        user_logs.log_actions(shared_state.current_user, action)

        purchase_list.clear(self)
        view.update_purchase_display(self)
        view.update_total_label(self)
        purchase_window.destroy(self)
    
    def check_admin_credentials(self, username, password, admin_login_window):
        user_status = Login_refactored.get_user_status(username)
        if user_status == "Inactive":
            messagebox.showerror("Error", "User is currently Inactive.\nContact your immediate Supervisor to Reactivate your account")
            user_logs.log_actions(username, action=f"{username} tried to return items but their account is inactive")
            return
        
        salt, stored_hashed_password = Login_refactored.get_stored_hashed_password(username)

        if salt and stored_hashed_password:
            hashed_password = Login_refactored.hash_password(password, salt)
            if hashed_password == stored_hashed_password:
                loa = Login_refactored.get_loa(username)
                if loa == "admin":
                    if user_status == "Active":
                        messagebox.showinfo("Success", "Login successful!")
                        admin_login_window.destroy(self)
                        import return_item
                        return_item.create_return_item_window(self)
                    else:
                ...
                user_logs.log_actions(username, action=f"{username} tried to log in but the entered password is incorrect")
        else:
            messagebox.showerror("Error", "Username not found.")
            user_logs.log_actions(username, action=f"{username} tried to log in but this username is not registered")
...
class pos_View:
    ...
    def __init__(self, controller):
        self.controller = controller
        self.create_pos_window

    def create_pos_window(self):
        ...
        self.center_window(window, 1280, 800)
        ...
        barcode.bind("<Return>", self.on_barcode_entry)  # Bind the Return (Enter) key to trigger the search
        ...
        logout_button = Button(text="Logout", font=("Hanuman Regular", 16), command=lambda: controller.go_to_window("logout"), bg="#FFFFFF", relief="raised")
        logout_button.place(x=1071.0, y=691.0, width=168.86373901367188, height=44.19459533691406)

        help_button = Button(text="Help", font=("Hanuman Regular", 16), command=lambda: controller.go_to_window("help"), bg="#FFFFFF", relief="raised")
        help_button.place(x=1071.0, y=623.0, width=168.86373901367188, height=44.19459533691406)

        purchase_button = Button(text="Purchase", font=("Hanuman Regular", 20), command=self.open_purchase_window, bg="#83F881", relief="raised")
        purchase_button.place(x=42.0, y=623.0, width=464.28277587890625, height=112.0)

        inventory_button = Button(text="Inventory", font=("Hanuman Regular", 20), command=lambda: controller.go_to_window("inventory"), bg="#81CDF8", relief="ridge")
        inventory_button.place(x=699.0, y=623.0, width=170.28277587890625, height=112.0)

        manual_button = Button(text="Manual Input", font=("Hanuman Regular", 16), command=self.manual_input, bg="#FFFFFF", relief="raised")
        manual_button.place(x=849.0, y=260.0, width=130, height=50)

        # Add Quantity button
        add_quantity_button = Button(text="Quantity", font=("Hanuman Regular", 16), command=self.add_quantity, bg="#FFFFFF", relief="raised")
        add_quantity_button.place(x=699.0, y=260.0, width=100, height=50)

        return_item_button = Button(text="Return Item", font=("Hanuman Regular", 16), command= controller.go_to_return, bg="#FFFFFF", relief="raised")
        ...
        if loa == "admin":
            ...
            register_button = Button(text="Accounts", font=("Hanuman Regular", 20), command=lambda: controller.go_to_window("register"), bg="#81CDF8", relief="ridge")
            register_button.place(x=699.0, y=477.0, width=170.28277587890625, height=112.0)

            reports_button = Button(text="Reports", font=("Hanuman Regular", 20), command=lambda: controller.go_to_window("reports"), bg="#81CDF8", relief="ridge")
            reports_button.place(x=884.0, y=477.0, width=170.28277587890625, height=112.0)

            backup_restore = Button(text="Backup\nRestore", font=("Hanuman Regular", 20), command=lambda: controller.go_to_window("backup_restore"), bg="#81CDF8", relief="ridge")
            backup_restore.place(x=1068.0, y=477.0, width=170.28277587890625, height=112.0)

        barcodes_button = Button(text="Barcode", font=("Hanuman Regular", 20), command=lambda: controller.go_to_window("barcode"), bg="#81CDF8", relief="ridge")
        barcodes_button.place(x=884.0, y=623.0, width=170.28277587890625, height=112.0)
        
        void_button = Button(text="Void", font=("Hanuman Regular", 20), command=lambda: self.confirm_void_action(purchase_list, self.update_purchase_display, self.update_total_label), bg="#FF9E9E", relief="raised")
        ...
        canvas.tag_bind(about, "<Button-1>", lambda event: controller.go_to_window("about"))
        ...
        canvas.tag_bind(users_manual, "<Button-1>", lambda event: controller.go_to_window("manual"))
    ...
    def center_window(self, curr_window, win_width, win_height):
    ...
    def open_purchase_window(self):
        ...
        self.center_window(purchase_window, 400, 300)
        ...
        tk.Button(purchase_window, text="Process Purchase", command=lambda: controller.process_purchase(cashier_name, customer_name_entry.get(), customer_contact_entry.get(), customer_money_entry.get(), purchase_window)).pack(pady=20)
    
    def prompt_admin_credentials(self):
        # Create a toplevel window for entering credentials
        admin_login_window = tk.Toplevel()
        admin_login_window.title("Admin Login")
        admin_login_window.geometry("150x100")

        self.center_window(admin_login_window, 150, 100)
        ...
        # Login button
        login_button = tk.Button(admin_login_window, text="Login", command=lambda: controller.check_admin_credentials(username_entry.get(), password_entry.get(), admin_login_window))
        login_button.pack()
    
    def create_receipt(self, cashier_name, customer_name, customer_contact, customer_money, change, purchase_list):
...
********************************
 ...
 THIS DOCUMENT IS NOT VALID FOR
      CLAIM OF INPUT TAX
        ...
        controller.print_receipt(receipt_text)

    def on_barcode_entry(event):
        """Handles the event when a barcode is entered."""
        barcode_value = barcode.get()
        controller.process_barcode(barcode_value)
        barcode.delete(0, 'end')  # Clear the barcode entry after processing

    def confirm_void_action(self,purchase_list, update_purchase_display, update_total_label):
    ...
    def manual_input(self):
        ...
        if manual_barcode:
            controller.process_barcode(manual_barcode)
...
if __name__ == "__main__":
    ...
    view = pos_View(controller)
    model = pos_Model(controller)
    view.create_pos_window()
